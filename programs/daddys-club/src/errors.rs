...
pub enum ClubError {
    ...
    /// `FR-021`: гасити достроково можна лише живе зобов'язання. До видачі
    /// його ще немає (`FR-012`), а в недозібраному випуску не буде вже ніколи
    /// (`FR-011`). Перехоплення цю відмову не кидає й кидати не може: воно
    /// вбудоване в чужу інструкцію збору комісій і тому влаштоване фільтром —
    /// стан випуску вирішує там, розщеплювати потік чи пропустити повз
    /// (`FR-014`).
