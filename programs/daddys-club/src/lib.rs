...
pub mod daddys_club {
    ...
    /// Виплачує власникові накопичене за його бондом (`FR-016`): різниця
    /// кумулятивного індексу (`FR-015`) від його останнього чекпоінта,
    /// помножена на баланс, плюс нараховане гуком при передачах. Стан випуску
    /// нічого не вирішує — забирати можна в будь-який момент, і найчастіше це
    /// роблять уже після повного погашення.
    pub fn claim(ctx: Context<Claim>) -> Result<()> {
        instructions::invest::claim(ctx)
    }

    /// Гасить залишок зобов'язання одним внеском емітента (`FR-021`). Купон
    /// при цьому сплачується повністю: зобов'язання зафіксоване при створенні
    /// (`FR-018`) і від швидкості погашення не залежить — дострокове погашення
    /// звільняє потік, а не здешевлює борг. Гроші йдуть у те саме сховище
    /// погашення, що й перехоплена частка, а випуск виходить звідси в `Repaid`,
    /// тому наступні комісії перехоплення пропускає цілими (`FR-019`).
    pub fn prepay(ctx: Context<Prepay>) -> Result<()> {
        instructions::issue::prepay(ctx)
    }

    /// Приймає дохід від зареєстрованого джерела й розщеплює його в тій самій
    /// транзакції: узгоджена частка — у сховище погашення, решта лишається
    /// емітенту (`FR-014`, `FR-020`). Викликача автентифікує **підпис** PDA
    /// програми-емітента (`FR-004`). Історію джерело накопичує завжди, навіть
    /// без випуску (`FR-028`); розщеплення ж діє, поки випуск погашається, і
